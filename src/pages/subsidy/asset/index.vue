<template>
  <div class="container">
    ...
    <div class="cate-c">
      <span class="c-l" :style="{'color': pageIndex === 1 ? lineStyle : null}" style="text-align:center;width:50%;" @click="updateList(1)">订单明细</span>
      <span class="c-m" :style="{'color': pageIndex === 2 ? lineStyle : null}" style="text-align:center;width:50%;" @click="updateList(2)">每日明细</span>
    </div>
      ...
      <div v-if="pageIndex === 2">
        ...
        <div class="cashlogs" v-for="(item, index) in orderList.datas" :key="index">
          <div class="profile">
            ...
            <div class="dateAdd">订单：{{item.count}} {{item.refund_status?',退单：' + item.refund_status : ''}}</div>
          </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      updateStatistics() {
        getFetch(
          "/order/statistics/" + this.userInfo.shopId, { page: this.orderList.page },
          true
        ).then(response => {
          var result = response.result || {};
      ...
      updateDetail() {
        getFetch(
          "/order/detail/" + this.userInfo.shopId, { page: this.orderList.page },
          true
        ).then(response => {
          var result = response.result || {};
          this.orderList.datas = [...this.orderList.datas, ...result.list];
