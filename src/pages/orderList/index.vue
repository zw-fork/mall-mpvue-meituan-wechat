<template>
  <div class="container">
    <div class="header-c">
      <div class="cate-c">
        <span class="c-l"
              ...
              @click="updateOrderList([])">全部订单</span>
        <span class="c-m"
    ...
    <scroll-view class="list-c"
                 ...
                 @scroll="scroll">
      <div class="item"
           v-for="(item, index) in orderList.datas"
           :key="index">
        <div class="shop-info">
          <div class="h-l">
            <i style="font-size:30rpx;color:#d81e06;"
               class="shop-logo icon iconfont icondianpu"></i>
          </div>
          <div class="order_title"
            ...
            <div class="order-name"
              ...
              <span class="shop-name"
                    style="display: inline">{{item.shopInfo.shopName}}</span>
          ...
          <p class="order-status"
             ...
             v-if="item.refundStatus==1">等待商家处理退款</p>
          <p class="order-status"
             style="position: absolute;right: 0;"
          ...
          <p class="order-status"
             style="position: absolute;right: 0;"
             v-else-if="item.status==0">待支付</p>
          <p class="order-status"
             style="position: absolute;right: 0;"
             v-else-if="item.status==1">待支付</p>
          <p class="order-status"
             style="position: absolute;right: 0;"
             v-else-if="item.status==2">已支付，等待商家配送</p>
          <p class="order-status"
             style="position: absolute;right: 0;"
          ...
          <p class="order-status"
             style="position: absolute;right: 0;"
             v-else-if="item.status==5">其他</p>
          <p class="order-status"
             style="position: absolute;right: 0;"
             v-else-if="item.status==6">退款申请已取消</p>
          <p class="order-status"
             style="position: absolute;right: 0;"
             v-else-if="item.status==7">退款申请中</p>
          <p class="order-status"
             style="position: absolute;right: 0;"
             v-else-if="item.status==8">已退款</p>
        </div>
        <div class="googs-c"
        ...
        <div class="bottom-c">
          ...
          <div class="btn"
               @click="cancelClick(item)"
               v-if="(!item.refundStatus || item.refundStatus == 3) && item.status>1">
            <span>退款</span>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    updateOrderList(status) {
      ...
      if (status.length < 1) {
        ...
        this.getOrderDataAction({
          uid: this.userInfo.openid,
          data: { page: 1, statusList: this.statusList.join(',') }
        });
      }
    },
    scroll(e) {
